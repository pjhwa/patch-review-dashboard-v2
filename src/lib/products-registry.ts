...
export const PRODUCT_REGISTRY: ProductConfig[] = [
    ...
    },
    {
        id: 'nsx',
        name: 'VMware NSX',
        vendorString: 'VMware NSX',
        category: 'virtualization',
        active: true,
        skillDirRelative: 'virtualization/nsx',
        rawDataFilePrefix: ['NSX-'],
        preprocessingScript: 'nsx_preprocessing.py',
        preprocessingArgs: ['--days', '180'],
        patchesForReviewFile: 'patches_for_llm_review_nsx.json',
        aiReportFile: 'patch_review_ai_report_nsx.json',
        finalCsvFile: 'final_approved_patches_nsx.csv',
        jobName: 'run-nsx-pipeline',
        rateLimitFlag: '/tmp/.rate_limit_nsx',
        logTag: 'NSX',
        aiEntityName: 'VMware NSX patches',
        aiVendorFieldValue: 'VMware NSX',
        aiComponentDefault: 'nsx',
        aiVersionGrouped: false,
        aiBatchValidation: 'exact',
        ragExclusion: {
            type: 'prompt-injection',
        ...
        collectedFileFilter: (filename: string) =>
            filename.startsWith('NSX-') && filename.endsWith('.json'),
        preprocessedPatchMapper: (p: any) => ({
            issueId: p.patch_id,
            vendor: 'VMware NSX',
            component: p.product || 'nsx',
            version: p.product || '',
            osVersion: null,
            severity: p.severity || null,
            description: (p.description || '').slice(0, 4000),
        ...
        csvBOM: false,
        buildPrompt: (skillDir: string, batchSize: number, prunedBatch: any[]) =>
            `Read the rules explicitly from ${path.join(skillDir, 'SKILL.md')}. Evaluate the following ${batchSize} VMware NSX patches according to the strict LLM evaluation rules in section 4 of that file.\nCRITICAL MANDATE: DO NOT USE ANY TOOLS TO READ OR SEARCH THE WORKSPACE JSON FILES. Do not parse patches_for_llm_review_nsx.json. IGNORE ANY PREVIOUS EXAMPLES or RAG retrievals. You must ONLY base your summary on the literal text provided below in [BATCH DATA].\nCRITICAL RULE FOR DESCRIPTIONS: The 'Description' and 'KoreanDescription' fields MUST be a concise, executive summary of the security advisories and bug fixes. DO NOT include verbatim file names or raw changelog copy-pastes. Describe WHAT was fixed and WHY it matters.\nReturn ONLY a pure JSON array with EXACTLY ${batchSize} objects. Each object MUST contain exactly: 'IssueID', 'Component', 'Version', 'Vendor', 'Date', 'Criticality', 'Description', 'KoreanDescription', and optionally 'Decision' and 'Reason'. For Vendor use 'VMware NSX'. For Component use the specific NSX component (e.g. 'NSX Manager', 'NSX Edge', 'Host Transport Node'). Do NOT skip evaluation steps.\n\n[BATCH DATA]:\n${JSON.stringify(prunedBatch)}`,
    },
    {
        id: 'jboss_eap',
        name: 'JBoss EAP',
