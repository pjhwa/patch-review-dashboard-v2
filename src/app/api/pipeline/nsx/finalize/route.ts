...
export async function POST(req: Request) {
    try {
        ...
        const nsxSkillDir = path.join(os.homedir(), '.openclaw/workspace/skills/patch-review/virtualization/nsx');
        const sourceJsonPath = path.join(nsxSkillDir, 'patch_review_ai_report_nsx.json');
        const outputCsvPath = path.join(nsxSkillDir, 'final_approved_patches_nsx.csv');
        ...
        return NextResponse.json({
            success: true,
            message: `Finalized ${approvedPatches.length} VMware NSX patches.`,
