...
export async function POST(req: Request) {
    try {
        ...
        const pgsqlSkillDir = path.join(os.homedir(), '.openclaw/workspace/skills/patch-review/database/pgsql');
        const sourceJsonPath = path.join(pgsqlSkillDir, 'patch_review_ai_report_pgsql.json');
        const outputCsvPath = path.join(pgsqlSkillDir, 'final_approved_patches_pgsql.csv');
        ...
        return NextResponse.json({
            success: true,
            message: `Finalized ${approvedPatches.length} PostgreSQL patches.`,
