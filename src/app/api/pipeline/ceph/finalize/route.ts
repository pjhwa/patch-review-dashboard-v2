...
export async function POST(req: Request) {
    try {
        ...
        const cephSkillDir = path.join(os.homedir(), '.openclaw/workspace/skills/patch-review/storage/ceph');
        const sourceJsonPath = path.join(cephSkillDir, 'patch_review_ai_report_ceph.json');
        const outputCsvPath = path.join(cephSkillDir, 'final_approved_patches_ceph.csv');
        ...
        return NextResponse.json({
            success: true,
            message: `Finalized ${approvedPatches.length} Ceph patches.`,
