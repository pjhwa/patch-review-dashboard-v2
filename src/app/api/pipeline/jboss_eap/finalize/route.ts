...
export async function POST(req: Request) {
    try {
        ...
        const jbossEapSkillDir = path.join(os.homedir(), '.openclaw/workspace/skills/patch-review/middleware/jboss_eap');
        const sourceJsonPath = path.join(jbossEapSkillDir, 'patch_review_ai_report_jboss_eap.json');
        const outputCsvPath = path.join(jbossEapSkillDir, 'final_approved_patches_jboss_eap.csv');
        ...
        return NextResponse.json({
            success: true,
            message: `Finalized ${approvedPatches.length} JBoss EAP patches.`,
