...
export async function POST(request: Request) {
    try {
        ...
        try {
            ...
            if (waitingJobs > 0) {
                console.log(`[JBOSS_EAP] Found ${waitingJobs} stuck waiting jobs. Clearing queue...`);
                await pipelineQueue.clean(0, 1000, 'wait');
            } else if (activeJobs > 0) {
                console.log(`[JBOSS_EAP] ${activeJobs} active job(s) detected — withOpenClawLock handles stale lock cleanup automatically.`);
            }
