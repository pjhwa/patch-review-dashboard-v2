...
export async function POST(req: Request) {
    try {
        ...
        const wildflySkillDir = path.join(os.homedir(), '.openclaw/workspace/skills/patch-review/middleware/wildfly');
        const sourceJsonPath = path.join(wildflySkillDir, 'patch_review_ai_report_wildfly.json');
        const outputCsvPath = path.join(wildflySkillDir, 'final_approved_patches_wildfly.csv');
        ...
        return NextResponse.json({
            success: true,
            message: `Finalized ${approvedPatches.length} WildFly patches.`,
