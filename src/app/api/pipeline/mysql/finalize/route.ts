...
export async function POST(req: Request) {
    try {
        ...
        const mysqlSkillDir = path.join(os.homedir(), '.openclaw/workspace/skills/patch-review/database/mysql');
        const sourceJsonPath = path.join(mysqlSkillDir, 'patch_review_ai_report_mysql.json');
        const outputCsvPath = path.join(mysqlSkillDir, 'final_approved_patches_mysql.csv');
        ...
        return NextResponse.json({
            success: true,
            message: `Finalized ${approvedPatches.length} MySQL Community patches.`,
