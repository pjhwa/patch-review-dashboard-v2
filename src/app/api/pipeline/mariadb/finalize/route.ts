...
export async function POST(req: Request) {
    try {
        ...
        const mariadbSkillDir = path.join(os.homedir(), '.openclaw/workspace/skills/patch-review/database/mariadb');
        const sourceJsonPath = path.join(mariadbSkillDir, 'patch_review_ai_report_mariadb.json');
        const outputCsvPath = path.join(mariadbSkillDir, 'final_approved_patches_mariadb.csv');
        ...
        return NextResponse.json({
            success: true,
            message: `Finalized ${approvedPatches.length} MariaDB patches.`,
