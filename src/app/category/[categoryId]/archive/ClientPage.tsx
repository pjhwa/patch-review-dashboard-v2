    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 min-h-screen">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div className="flex items-center gap-4">
                    <Link
                        ...
                        className="p-2 rounded-full border border-white/5 bg-white/[0.02] hover:bg-white/10 hover:border-white/20 transition-all flex-shrink-0"
                    >
                    ...
                    <div>
                        ...
                        <p className="text-white/50 text-sm mt-1">{d.quarterlySubtitle}</p>
                    </div>
                </div>
                <button
            ...
            {showCreate && (
                <div className="p-6 border border-emerald-500/20 rounded-xl bg-emerald-500/[0.04] space-y-4">
                    <div>
                        ...
                        <p className="text-white/40 text-xs mt-1">{d.createArchiveDesc}</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                        <div className="flex flex-col gap-1">
                            <label className="text-white/50 text-xs">{d.quarterLabel}</label>
                            <input
                                type="text"
                                value={newQuarter}
                                onChange={e => { setNewQuarter(e.target.value); setCreateError(''); }}
                                placeholder={d.quarterPlaceholder}
                                className="w-40 px-3 py-2 bg-black border border-white/10 rounded-lg text-white text-sm focus:border-emerald-500/40 focus:outline-none placeholder:text-white/20"
            ...
            {loading ? (
                ...
                <div className="p-16 text-center border-dashed border border-white/10 rounded-xl bg-white/[0.01]">
                    <Archive className="w-12 h-12 text-white/15 mx-auto mb-4" />
                    <p className="text-white/40 font-medium mb-2">{d.noQuarters}</p>
                    <p className="text-white/25 text-xs">{d.noQuartersSub}</p>
                </div>
            ) : (
                <Tabs defaultValue={quarters[0]?.dirName || quarters[0]?.quarter.replace(' ', '-')} className="w-full">

                    {/* Quarter Tabs */}
                    <TabsList className="bg-black border border-white/10 mb-6 p-1 h-auto flex flex-wrap gap-1">
                        {quarters.map(q => {
                            ...
                            return (
                                ...
                                >
                                    {q.quarter}
                                    {catPatchCount > 0 && (
                                        <span className="ml-2 px-1.5 py-0.5 text-[10px] rounded bg-white/[0.06] text-white/40">
                                            {catPatchCount}
                                        </span>
                                    )}
                                </TabsTrigger>
                    ...
                    {quarters.map(q => {
                        ...
                        return (
                            <TabsContent key={q.quarter} value={tabKey} className="mt-0 space-y-4">
                                ...
                                {/* Archive Meta Bar */}
                                <div className="flex flex-wrap items-center gap-3 px-4 py-3 rounded-lg bg-white/[0.02] border border-white/5">
                                    <div className="flex items-center gap-2">
                                        <Archive className="w-4 h-4 text-white/30" />
                                        <span className="text-white/60 text-sm font-medium">{q.quarter}</span>
                                    </div>
                                    <div className="h-3 w-px bg-white/10"></div>
                                    <Badge variant="outline" className="border-white/10 text-white/40 bg-transparent text-xs">
                                ...
                                {categoryProducts.length === 0 ? (
                                    <div className="p-12 text-center text-white/30 border border-dashed border-white/10 rounded-lg bg-black/50">
                                        {d.noHistory}
                                        <br /><span className="text-xs mt-2 block text-white/20">{d.noHistorySub}</span>
                                    </div>
                                ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 pt-2">
                                        {categoryProducts.map((prod: any) => (
                                            <Card
                                                key={prod.productId}
                                                className="relative overflow-hidden border-white/[0.06] bg-[#0a0a0a] transition-all duration-300 hover:border-white/20 hover:bg-white/[0.04] hover:shadow-[0_0_30px_-5px_rgba(255,255,255,0.05)]"
                                            >
                                                ...
                                                <CardContent className="pt-4 relative">
                                                    ...
                                                    <div className="text-xs text-white/25 mb-3 font-mono truncate">
