...
### 전략 1: 프롬프트 주입 (Red Hat, Oracle, Ubuntu)

첫 번째 배치 실행 전:

1. `query_rag.py`가 현재 세션의 패치 요약을 입력으로 호출됨
...
**Linux에서 파일 숨김 대신 프롬프트 주입을 사용하는 이유**: `query_rag.py`가 단순한 파일 숨김보다 더 세밀한 유사도 기반 배제를 제공합니다.
...
**이유**: OpenClaw `agent:main`이 파일 도구를 통해 워크스페이스 파일 시스템에 접근할 수 있습니다. 정규화된 데이터 디렉터리를 숨기지 않으면 에이전트가 이전에 처리된 파일을 읽어 편향된 결과를 생성할 수 있습니다.

### RAG 없음 (VMware vSphere)

vSphere는 `ragExclusion` 설정 없음 — 매 파이프라인 실행마다 전처리된 패치 전체를 새로 검토합니다.
...
**패스스루 적용 대상**: 버전 그룹핑을 사용하는 Windows Server와 SQL Server를 제외한 모든 제품.
