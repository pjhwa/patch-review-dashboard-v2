...
# NOTE: Preprocesses VMware NSX patch data files (NSX-*.json) for AI review.
# Two types of files:
#   - security_advisory: VMSA advisories with CVEs
#   - update_release: Build/Update releases with bug fixes
# Collector: nsx_collector.py
# Products: NSX 4.1/4.2, NSX-T Data Center 3.2

DATA_DIR = "nsx_data"
OUTPUT_FILE = "patches_for_llm_review_nsx.json"
AUDIT_LOG_FILE = "dropped_patches_audit_nsx.csv"
...
def build_description(data):
    ...
    if patch_type == 'security_advisory':
        ...
        upgrade_order = data.get('upgrade_order', [])
        if upgrade_order:
            parts.append("Upgrade order: " + " | ".join(upgrade_order))
        ...
        if version:
            parts.append(f"[UPDATE RELEASE] {product} {version} (Build {build})")
        ...
        if high_noncve:
            ...
            for f in high_noncve[:5]:
                ...
                parts.append(f"[{sev}]{comp_str} {desc[:400]}")
    ...
    if not parts:
        parts.append(data.get('type', 'VMware NSX Update'))
...
def preprocess_patches():
    ...
    json_files = sorted(glob.glob(os.path.join(DATA_DIR, "NSX-*.json")))
    ...
    if os.path.exists(db_path):
        try:
            ...
            cursor.execute("DELETE FROM PreprocessedPatch WHERE vendor = 'VMware NSX'")
...
if __name__ == '__main__':
    ...
    print(f"[NSX-PREPROCESS_DONE] count={count}")
