...
- `oracle/oracle_parser.py` — Parses updateinfo.xml and writes ELSA JSON files
...
- Oracle mirrors RHEL advisories closely, but uses ELSA numbering (not RHSA)
...
**What this step does:**
1. Reads JSON files from `oracle/oracle_data/` (files prefixed `ELSA-`)
...
- For `OsVersion` field: preserve the `os_version` field as-is (e.g., `"Oracle Linux 8"`).
...
- NEVER confuse ELSA advisory numbers with RHSA numbers.
...
- **IssueID format**: `"ELSA-2026-1234"` (hyphen separator, not colon).
...
- **OsVersion**: String such as `"Oracle Linux 8"` or `"Oracle Linux 8, Oracle Linux 9"` — from `os_version` field.
...
3. `IssueID` matches the source advisory ID exactly (e.g., `ELSA-2026-1234` with hyphens, not colons).
