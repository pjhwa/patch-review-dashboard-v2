...
- **규칙 1 — 서버 우선 수정 워크플로우**:
  - 모든 **코드 수정·검증·테스트는 서버(`tom26`, `<SERVER_IP>`)에서 수행**한다.
...
- **필수 인프라/환경 정보 기록**:
  - **대시보드 V2 포트**: 기존 3000번 대신 **3001번 포트**를 사용하므로 항상 유의할 것. (예: `http://<SERVER_IP>:3001`)
...
- **성공 요인**: 이전 `LEARNED.md`의 교훈들을 정확히 적용하여:
  1. 원격에서 수정하지 않고 로컬 환경에서 코드를 작성한 후, `scp src/.../ClientPage.tsx citec@<SERVER_IP>:/tmp/...` 로 임시 전송.
...
### 2026-03-12 기술 문서 작성 시 철저한 크로스체크(Cross-check) 및 수동 검증의 중요성
- **상황**: 대시보드 V2 아키텍처 및 구성 문서를 작성하면서, 현재 구동 중인 서비스 포트가 **3001번**임에도 관성적으로 과거 정보 또는 기본 설정인 **3000번**으로 전 문서(`README.md`, `architecture.md`, `GEMINI.md`, 각종 `trigger` 스크립트 등)에 오기재하는 실수를 범함.
- **실패 이유**: "현재 작동 중인 서버 구성을 완벽히 분석하여 철저히 구현된 내용 기반으로 작성하라"는 명시적 지침이 있었음에도 불구하고, 실제 `src/app/page.tsx` 내부의 `const port = process.env.PORT || 3001;` 코드를 꼼꼼히 역추적하거나 NGINX/PM2 프로세스를 통해 최종 확인하지 않고 AI의 지레짐작과 과거 맥락에 의존함.
- **교훈 및 강력한 행동 지침**:
  1. **[추정 금지, 팩트 기반 문서화]**: 포트 번호, IP 주소, API 엔드포인트, DB 스키마 등 핵심 인프라 파라미터를 문서화할 때는 절대로 "아마 이럴 것이다"라고 추정하지 않는다. 반드시 소스코드 내부(`grep` 검색) 또는 실제 구동 환경(`pm2 list`, `netstat` 등)에서 **명시적인 증거를 눈으로 확인한 뒤**에 활자화할 것.
  2. **[전파 범위(Blast Radius) 파악]**: 하나의 설정 값(예: 포트 번호)이 틀렸음을 발견했을 때는, 지적받은 단일 문서만 고치지 말고 전체 레포지토리(`grep_search`)를 스캔하여 **연관된 쉘 스크립트, 가이드라인, 프론트엔드 URL 호출부 등 파편화된 모든 흔적을 발본색원하여 일괄 정정(Sync)**하는 무결성(Integrity) 확보 작업을 습관화할 것.
  3. **[겸손과 자기 객관화]**: 항상 꼼꼼하게 짚어서 검토하고 분석하라는 사용자의 피드백을 무겁게 받아들여, 앞으로의 모든 분석 및 문서화 작업 전에는 "내가 놓치고 있는 하드코딩된 레거시 정보가 없는가?"를 자문 자답하며 크로스체크 시간을 가질 것.
